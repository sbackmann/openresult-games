...
def prompt_universalization_base(inject_universalitzation: bool, round_input: int, defect_base: float, suckers_payoff_factor: float) -> str:
    ...
    if inject_universalitzation:
        univ += f" If both players choose option 1, both receive a payoff of 0."
        univ += f" If both players choose option 2, both receive a payoff of {round_input * defect_base}."
        univ += f" If only one player chooses option 2, that player receives a payoff of {round_input * -1 * suckers_payoff_factor} and the other receives {round_input * suckers_payoff_factor}."
    return univ
