...
from .environment import ChickenPerturbationEnv
from .persona import ChickenPersona, ChickenDummyPersona
...
def run(
    ...
    default_options = {
        "z-gpt-4o-mini-2024-07-18": ("GPT-4o-mini", "1"),
        "meta-llama/llama-3.3-70b-instruct": ("Llama-3.3-70B", "1"),
        "z-gpt-o3-mini-2025-01-31": ("OpenAI o3-mini", "1"),
        "deepseek/deepseek-r1": ("Deepseek-R1", "1"),
    }
    ...
    for i in range(num_personas):
        personas.update({
            ...
            else ChickenDummyPersona(cfg.agent, wrappers[i], actions, cfg.env.max_num_rounds)
        })
        cognition_utils.DEFAULT_OPTIONS.update({f"persona_{i}": default_options[wrappers[i].model_path]})

    agent_name_to_id, agent_id_to_name = init_all_personas(personas, num_personas, cfg)

    env = ChickenPerturbationEnv(cfg.env, experiment_storage, agent_id_to_name, seed=seed)
    ...
    for persona in personas:
        if isinstance(personas[persona], ChickenPersona):
